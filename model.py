...
def explain_flatten():
    ...
    print(
        "- end_dim (optional): The last dimension to flatten. Default is -1 (last dimension)"
    )
    print()
    ...
    print(
        "It reshapes the tensor by combining the specified dimensions into a single dimension."
    )
    print()
    ...
    print(
        "This flattens dimensions 1 and 2 (3*4=12), keeping dimensions 0 and 3 separate"
    )
    ...
    flattened = torch.flatten(
        conv_output, 1
    )  # Flatten from dimension 1 (keep batch separate)
    ...
    print(
        "The batch dimension (4) is preserved, and the rest [512, 7, 7] becomes [512*7*7]"
    )
...
if __name__ == "__main__":
    ...
    trainable_params = sum(
        p.numel() for p in model_imagenet.parameters() if p.requires_grad
    )
